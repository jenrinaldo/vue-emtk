<template>
  <div class="m-fnt">
    ...
    <h1 class="staggeredAnim content-title">Info Beasiswa</h1>
    <h6 class="staggeredAnim content-caption">Jangan sampai ketinggalan</h6>
    ...
    <div class="">
      <isotope ref="cpt" v-bind:options="isotopeOptions()" v-bind:list="list" v-on:layout="doneLayout" class="grid-isotope">
          <div v-for="data,index in list" v-bind:class="data.class" v-bind:key="index">
            <router-link v-bind:to="'/beasiswa/'+data.link" v-if="data.class == 'grid-item'" class="center-align staggeredAnimIso">
              <h5>{{ data.title }}</h5>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      getPost () {
        this.postLoad = true;
        this.$http.get('/public/posts/feeds.php?c=beasiswa&t=list&l=50&o=0').then(response => {
          if (!response.body.list) {
            this.errorMsg('Tidak ada beasiswa.');
          } else {
