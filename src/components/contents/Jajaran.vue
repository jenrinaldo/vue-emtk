<template>
	<div class="m-fnt">
		<div class="hexa hexa-left staggeredAnim"><img src="/public/assets/hexa.svg" height="54px"></div>
        <div class="hexa hexa-right staggeredAnim"><img src="/public/assets/hexa.svg" height="54px"></div>
		<h1 class="staggeredAnim content-title">JAJARAN</h1>
		<h6 class="staggeredAnim content-caption">EMTK</h6>
		<div class="row center-md">
			<div class="col-xs-12 col-sm-6 col-md-4 center-align">
				<router-link to="/emtk/ketua-wakil" exact class="dept-hexa staggeredAnim">
					<svg version="1.1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="280" height="280">
						<defs>
							<clipPath id="textbottom">
								<polygon points="0,160 280,190 280,280 0,280" />
							</clipPath>
							<clipPath id="foto">
								<polygon points="0,150 0,66 140,0 280,66 280,180" />
							</clipPath>
						</defs>
						<polygon points="140,0 280,66 280,214 140,280 0,214 0,66" clip-path="url(#textbottom)" class="bottom-text" />
						<image xlink:href="/public/images/departemen/waka.jpg" x="0" y="0" width="280px" clip-path="url(#foto)"/>
						<text transform="rotate(6)" x="140" y="185" text-anchor="middle" class="deptnama">ketua &amp; wakil ketua</text>
					</svg>
				</router-link>
			</div>
			<div class="col-xs-12 col-sm-6 col-md-4 center-align">
				<router-link to="/emtk/bendahara-audit" exact class="dept-hexa staggeredAnim">
					<svg version="1.1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="280" height="280">
						<defs>
							<clipPath id="textbottom">
								<polygon points="0,160 280,190 280,280 0,280" />
							</clipPath>
							<clipPath id="foto">
								<polygon points="0,150 0,66 140,0 280,66 280,180" />
							</clipPath>
						</defs>
						<polygon points="140,0 280,66 280,214 140,280 0,214 0,66" clip-path="url(#textbottom)" class="bottom-text" />
						<image xlink:href="/public/images/departemen/bendahara.jpg" x="0" y="0" width="280px" clip-path="url(#foto)"/>
						<text transform="rotate(6)" x="140" y="185" text-anchor="middle" class="deptnama">bendahara &amp; audit</text>
					</svg>
				</router-link>
			</div>
		</div>
		<div class="staggeredAnim">
	          <h4 class="center-align">Departemen</h4>
	          <div class="staff-spacer"></div>
	        </div>
		<div class="row center-md">
			<div class="col-xs-12 col-sm-6 col-md-4 center-align">
				<router-link to="/emtk/adkest" exact class="dept-hexa staggeredAnim">
					<svg version="1.1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="280" height="280">
						<defs>
							<clipPath id="textbottom">
								<polygon points="0,160 280,190 280,280 0,280" />
							</clipPath>
							<clipPath id="foto">
								<polygon points="0,150 0,66 140,0 280,66 280,180" />
							</clipPath>
						</defs>
						<polygon points="140,0 280,66 280,214 140,280 0,214 0,66" clip-path="url(#textbottom)" class="bottom-text" />
						<image xlink:href="/public/images/departemen/adkest.jpg" x="0" y="0" width="280px" clip-path="url(#foto)"/>
						<text transform="rotate(6)" x="140" y="185" text-anchor="middle" class="deptnama">adkest</text>
					</svg>
				</router-link>
			</div>
			<div class="col-xs-12 col-sm-6 col-md-4 center-align">
				<router-link to="/emtk/advokesma" exact class="dept-hexa staggeredAnim">
					<svg version="1.1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="280" height="280">
						<defs>
							<clipPath id="textbottom">
								<polygon points="0,160 280,190 280,280 0,280" />
							</clipPath>
							<clipPath id="foto">
								<polygon points="0,150 0,66 140,0 280,66 280,180" />
							</clipPath>
						</defs>
						<polygon points="140,0 280,66 280,214 140,280 0,214 0,66" clip-path="url(#textbottom)" class="bottom-text" />
						<image xlink:href="/public/images/departemen/advo.jpg" x="0" y="0" width="280px" clip-path="url(#foto)"/>
						<text transform="rotate(6)" x="140" y="185" text-anchor="middle" class="deptnama">advokesma</text>
					</svg>
				</router-link>
			</div>
			<div class="col-xs-12 col-sm-6 col-md-4 center-align">
				<router-link to="/emtk/kwu" exact class="dept-hexa staggeredAnim">
					<svg version="1.1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="280" height="280">
						<defs>
							<clipPath id="textbottom">
								<polygon points="0,160 280,190 280,280 0,280" />
							</clipPath>
							<clipPath id="foto">
								<polygon points="0,150 0,66 140,0 280,66 280,180" />
							</clipPath>
						</defs>
						<polygon points="140,0 280,66 280,214 140,280 0,214 0,66" clip-path="url(#textbottom)" class="bottom-text" />
						<image xlink:href="/public/images/departemen/kwu.jpg" x="0" y="0" width="280px" clip-path="url(#foto)"/>
						<text transform="rotate(6)" x="140" y="185" text-anchor="middle" class="deptnama">kwu</text>
					</svg>
				</router-link>
			</div>
			<div class="col-xs-12 col-sm-6 col-md-4 center-align">
				<router-link to="/emtk/infokom" exact class="dept-hexa staggeredAnim">
					<svg version="1.1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="280" height="280">
						<defs>
							<clipPath id="textbottom">
								<polygon points="0,160 280,190 280,280 0,280" />
							</clipPath>
							<clipPath id="foto">
								<polygon points="0,150 0,66 140,0 280,66 280,180" />
							</clipPath>
						</defs>
						<polygon points="140,0 280,66 280,214 140,280 0,214 0,66" clip-path="url(#textbottom)" class="bottom-text" />
						<image xlink:href="/public/images/departemen/inf.jpg" x="0" y="0" width="280px" clip-path="url(#foto)"/>
						<text transform="rotate(6)" x="140" y="185" text-anchor="middle" class="deptnama">infokom</text>
					</svg>
				</router-link>
			</div>
			<div class="col-xs-12 col-sm-6 col-md-4 center-align">
				<router-link to="/emtk/psdm" exact class="dept-hexa staggeredAnim">
					<svg version="1.1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="280" height="280">
						<defs>
							<clipPath id="textbottom">
								<polygon points="0,160 280,190 280,280 0,280" />
							</clipPath>
							<clipPath id="foto">
								<polygon points="0,150 0,66 140,0 280,66 280,180" />
							</clipPath>
						</defs>
						<polygon points="140,0 280,66 280,214 140,280 0,214 0,66" clip-path="url(#textbottom)" class="bottom-text" />
						<image xlink:href="/public/images/departemen/psdm.jpg" x="0" y="0" width="280px" clip-path="url(#foto)"/>
						<text transform="rotate(6)" x="140" y="185" text-anchor="middle" class="deptnama">psdm</text>
					</svg>
				</router-link>
			</div>
			<div class="col-xs-12 col-sm-6 col-md-4 center-align">
				<router-link to="/emtk/sosma" exact class="dept-hexa staggeredAnim">
					<svg version="1.1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="280" height="280">
						<defs>
							<clipPath id="textbottom">
								<polygon points="0,160 280,190 280,280 0,280" />
							</clipPath>
							<clipPath id="foto">
								<polygon points="0,150 0,66 140,0 280,66 280,180" />
							</clipPath>
						</defs>
						<polygon points="140,0 280,66 280,214 140,280 0,214 0,66" clip-path="url(#textbottom)" class="bottom-text" />
						<image xlink:href="/public/images/departemen/sosma.jpg" x="0" y="0" width="280px" clip-path="url(#foto)"/>
						<text transform="rotate(6)" x="140" y="185" text-anchor="middle" class="deptnama">sosma</text>
					</svg>
				</router-link>
			</div>
		</div>
	      <div class="back-header"></div>
	</div>
</template>

<script>

	export default {
		data () {
			return {

			}
		},
		mounted () {
			setTimeout(function(){
				TweenMax.staggerTo('.staggeredAnim', .8, {
					x: 0,
					opacity: 1,
					ease: Power2.easeOut
				}, .1);
			},600)
		},
	    head: {
	      title: {
	        inner : 'Jajaran EMTK'
	      }
	    }
	}
</script>

<style scoped>
	h4 {
    	margin: 7px 0 15px 0;
    	color: #f8f8f8;
  	}


  	.dept-hexa {
  		width: 250px;
  		height: 250px;
  		display: inline-block;
  		margin-left: -5px;
  		margin: 25px 0;
  		perspective: 600px;
  	}

  	.dept-hexa svg {
  		transform: rotateY(0deg) scale(1);
  		transition: transform 0.5s cubic-bezier(0.77, 0, 0.175, 1);
  	}

  	.dept-hexa svg:hover {
  		transform: rotateY(30deg) scale(1.2);
  	}

  	.dept-hexa .bottom-text {
  		fill: #353E68;
  		transition: fill 0.5s cubic-bezier(0.77, 0, 0.175, 1);
  	}

  	.dept-hexa svg:hover .bottom-text {
  		fill: #025eb7;
  	}

  	.dept-hexa .deptnama {
  		text-transform: uppercase;
  		font-weight: 700;
  		fill: #C3C6D3;
  		font-size: 16px;
  		letter-spacing: 1px;
  	}
</style>